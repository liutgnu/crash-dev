...
{
  /* 1st, switch to tid_p 0 if we are not */	
  if (inferior_thread()->ptid.tid_p()) {
	switch_to_thread (current_inferior()->thread_list);
  }
  /* 2nd, delete threads whose tid_p is not 0 */
  for (thread_info *tp : current_inferior()->threads_safe()) {
	if (tp->ptid.tid_p() && tp->deletable()) {
		delete_thread_silent(tp);
		current_inferior()->highest_thread_num--;
	}
  }
  /* 3rd, refresh regcache for tid_p 0 */
  target_fetch_registers(get_current_regcache(), -1);
  reinit_frame_cache();
  return TRUE;
}
